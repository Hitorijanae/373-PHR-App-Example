...
$(document).ready(function(){
	...
	myDatabase = new myDB();
	myDatabase.appendDivId = "health_history";
	myDatabase.openDB();
...
var myDB = function(){
	...
	this.getHistory = function(){
		var self = this;
		this.db.transaction(function(tx){
			tx.executeSql('SELECT * FROM history', [], querySuccess);
		});
			function querySuccess(tx, results){
				myDatabase.db.transaction(function(tx){
					tx.executeSql("SELECT * FROM history",[], function(tx, results){
						...
						if(len>0){
							var htmlArray = [];
							for(var i = 0;i < len; i++){
								var html = "<li id = '" + results.rows.item(i).now + "'class = 'history_li'><span class = 'delete_item'>x</span><span class = 'h title'>" + results.rows.item(i).title + "</span><span class = 'h_notes'>" + "</span> </li>";
